    ...
    for n in range(1, T + U - 1):
        ...
        if u == 0:
            ...
            # for t in range(1, T) for u in range(1, U) step to compute alphas[b, t, u]
                ...
                alphas[offset + t * maxU + u] = log_sum_exp(emit, no_emit)
    ...
    for n in range(T + U - 2, -1, -1):
        ...
        if u == (U - 1):
            ...
            if t == T - 1:
                ...
                betas[offset + t * maxU + u] = log_sum_exp(emit, no_emit)
            ...
            if t > 0 and t < T:
                ...
                for i in range(num_big_blanks):
                    if t >= big_blank_duration[i]:
                        alphas[offset + t * maxU + u] = log_sum_exp(
                            alphas[offset + t * maxU + u],
    ...
    if u == 0:
        ...
        for i in range(num_big_blanks):
            if T >= big_blank_duration[i]:
                ...
                loglike = log_sum_exp(loglike, big_blank_loglike)
            ...
            if t >= 0 and t < (T - 1):
                ...
                for i in range(num_big_blanks):
                    if t + big_blank_duration[i] < T:
                        ...
                        betas[offset + t * maxU + U - 1] = log_sum_exp(
                            betas[offset + t * maxU + U - 1],
            ...
            if t == T - 1:
                ...
                for i in range(num_big_blanks):
                    if t < T - big_blank_duration[i]:
                        ...
                        betas[offset + t * maxU + u] = log_sum_exp(
