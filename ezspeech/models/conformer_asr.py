    ...
    def forward(
        ...
        for idx,layer in enumerate(self.conformer_layers):
            if idx in self.inter_layer:
                ...
                x=x+inter_layer_in
                x=x.transpose(1,0)
            elif idx in self.phoneme_inter_layer:
                x=layer(x,encoder_padding_mask)
                x=x.transpose(0, 1)
                inter_layer_out=self.phoneme_lm_head(x)
                inter_prediction= inter_layer_out.log_softmax(2)
                inter_layer_in=self.phoneme_back_to_hidden(inter_prediction)
                phoneme_inter_layer_softmax_lst.append(inter_prediction)
                x=x+inter_layer_in
                x=x.transpose(1,0)
            else:
                x = layer(x, encoder_padding_mask)
        ...
        return x, lengths,inter_layer_softmax_lst,phoneme_inter_layer_softmax_lst